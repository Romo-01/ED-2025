...
int main (int argc, char **argv) {
    ...
    node = dlist_head(&list);
     for (i = 1; i < argc; i++) {
        ...
        *data = atoi(argv[i]);  //aqui se lee cada argumento con el argv[i] y lo convierte a entero usando atoi porque hace que una cadena de caracteres se convierta en tipo int
        
        if (dlist_size(&list) == 0) { // para que siempre vayan a la tail 
